...
{
	std::uint32_t transformedColor =  XMCOLOR(color.b,color.g,color.r,color.a);
...
void Renderer::drawText(const Vec2& pos, const std::string& text, const Color& color, std::uint32_t flags,
	float fontSize, const std::wstring& fontFamily)
{
	std::wstring wsTmp(text.begin(), text.end());
	drawText(renderList, pos, wsTmp, color, flags, fontSize, fontFamily);
}

void Renderer::drawText(const Vec2& pos, const Color& color, std::uint32_t flags , float fontSize , const char* text, ...) {
	va_list va_alist;
	char logbuf[256];
	...
	drawText( pos, std::string(logbuf), color, flags, fontSize, L"");
}
