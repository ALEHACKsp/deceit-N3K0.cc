...
signed __int64 __fastcall GetFireRate(void* v,bool a2) {
	...
	*(__int16*)(rcx + 0x90) = (__int16)2000; // I have some problem with firerate idk why when i edit value from class it just access violation crash
	return CFireMode.get_orignial_function<signed __int64(__thiscall*)(void*, bool)>(102)(v,a2);
}
...
int __stdcall ray_function_hook(void* this_, SRWIParams* rp, const char* pNameTag, int iCaller) {
	...
	int result = physical.get_orignial_function<ray_function>(35)(sdk::global_var::pPhysicalWorld, rp, pNameTag, iCaller);
...
HRESULT __stdcall Hook::PresentHook(IDXGISwapChain* pSwapChain, UINT SyncInterval, UINT Flags) 
{
	...
	if (!hook_bullshit) {
		if (sdk::global_var::pPhysicalWorld) {
			physical.setup(sdk::global_var::pPhysicalWorld);
			physical.hook_virtual_function(35, ray_function_hook);
			hook_bullshit = true;
		}
	}
					...
					if (cur_weapon) {
						...
						if (cur_weapon->m_fm) {
							if (!cur_weapon_hook) { // I  can just detour jmp this thing but Im to lazy 
								...
								CFireMode.hook_virtual_function(26, canfire_hook);
								CFireMode.hook_virtual_function(102, GetFireRate);
							...
							if (misc::sort_mode == sort_type::distance)
								...
								if (sdk::global_var::pRender->ProjectToScreen(entity->m_position, &w2s)) {
									fl_distance = w2s.fl_distance(
										Vector(sdk::global_var::pRender->get_width() / 2.0f,
											   sdk::global_var::pRender->get_height() / 2.0f, 0));
								}
								else
									continue;
...
void __fastcall plog_log_hook(void*v, DWORD64 a ,const char* format, va_list va) {
	...
	printf("CryLog >> %s \n", logbuf);
	static plog_ log = CLog.get_orignial_function<void(__thiscall*)(void*, DWORD64, const char*, va_list)>(1);
	log(v, a, format, va);
}
...
DWORD Hook::Hook(HWND hwnd) {
	...
	CLog.hook_virtual_function(1, plog_log_hook);
...
void Hook::UnHook() {
	...
	if (CFireMode.should_unhook()) {
		CFireMode.unhook_virtual_function(26);
		CFireMode.unhook_virtual_function(102);
	}
	if (CLog.should_unhook()) {
		CLog.unhook_virtual_function(1);
	}
	if (physical.should_unhook()) {
		physical.unhook_virtual_function(35);
	}
}