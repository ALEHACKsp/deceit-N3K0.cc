...
#define width_offset 0x1B480C4
#define height_offset 0x1B480C8
...
public:
	...
	bool ProjectToScreen(float ptx, float pty, float ptz, float* sx, float* sy, float* sz)
	{
		...
		*sx *= ((float)get_width() / 100.0f);
		*sy *= ((float)get_height() / 100.0f);
		*sz *= 1.0f;
	...
	int get_width() {
		static DWORD64 game_base = (DWORD64)(GetModuleHandleA(0));
		return *(int*)(game_base + width_offset); // if it outdate just find it with cheat engine it not that hard
	}											  // ^
	int get_height() {							  // |
		static DWORD64 game_base = (DWORD64)	  /* | */(GetModuleHandleA(0));
		return *(int*)(game_base + height_offset);// | 
	}
};
#pragma endregion
...
void CryLog(const char* format, Args...args) {
	static auto import = (void(*)(const char*, ...))(((DWORD64)GetModuleHandleA(0) + 0xE9920));
